...
pub fn terrain_heal(this: Option<&Unit>, terrain: i64, method_info: OptionalMethod) -> i32 {
    const MAX_HP: i32 = 0;
    let heal = call_original!(this, terrain, method_info);
    if let Some(unit) = this {
        unit.get_capability(MAX_HP, true) * heal / 100
    } else {
        heal
    }
}
...
pub fn terrain_damage(this: Option<&Unit>, terrain: i64, method_info: OptionalMethod) -> i32 {
    const MAX_HP: i32 = 0;
    let dmg = call_original!(this, terrain, method_info);
    if let Some(unit) = this {
        unit.get_capability(MAX_HP, true) * dmg / 100
    } else {
        dmg
    }
}
...
pub fn main() {
    ...
    skyline::install_hooks!(terrain_damage, terrain_heal);
}
